...
static int health1 = 10;
static int health2 = 10;
...
static bool alreadyshot1 = false;
static bool alreadyshot2 = false;
static bool angflag1 = false;
static bool angflag2 = false;
...
{
	//player 1 goes first
	if (playerflag == false && lockflag == 0)
	{
	  if (key=='a')
	...
	else if (playerflag == true && lockflag2 == 0)
	{
	  if (key=='+')
	  {
		    pressed2 = 1;
...
{
	//player 2 can go
	if (playerflag == true && lockflag2 == 0)
	{
	  if (key==GLUT_KEY_LEFT)
          keys[0]=true;         // mark left as being pressed
     if (key==GLUT_KEY_RIGHT)
          keys[1]=true;         // mark right as being pressed
	  if (key==GLUT_KEY_UP)
	  {
		    if (angflag2 == false)
			 {
...
void processSpecialKeysUp(int key, int x, int y)
{
	...
	{
     if (key==GLUT_KEY_LEFT)
          keys[0]=false;         // mark left as being released
     if (key==GLUT_KEY_RIGHT)
          keys[1]=false;         // mark right as being released
	  if (key==GLUT_KEY_UP)
	  {
			 angflag2 = false;
          keys[2]=false;         // mark up as being released
	  }
     if (key==GLUT_KEY_DOWN)
	  {
		    angflag2 = false;
          keys[3]=false;         // mark down as being released
	  }
	}
}
...
{
	printf("Welcome to AK-47's Tanks!\n\nThis is a game for ");
	printf("2 players where you shoot your projectile and try to ");
	printf("hit the other player's tank. When the other tank's health");
	printf(" goes to zero, you win.\n\n");
	printf("Controls:\n\nPlayer 1\n");
	printf("a: move left\nd:move right\nw: aim up\ns: aim down\nspacebar: hold, release to shoot\n\n");
	printf("Player2\nleft arrow: move left\nright arrow: move right\n");
	printf("up arrow: aim up\ndown arrow: aim down\n+: hold, release to shoot.\n\n");
	printf("Have fun!\n\nPLAYER 1's TURN\n");
	glutInit(&argc, argv);
   ...
   for (int i = 0; i < NUM_PARTICLES; i++) 
	{
      ...
      pos[i][2] += vel[i][2] * TIMESTEP +
		...
		//printf("pos[i][0]: %f\n", pos[i][0]);
		//printf("sheet1trans[0]: %f\n", sheet1trans[0]);

		//printf("t2trans[0]: %f\n", t2trans[0]);
		if (abs(pos[i][0] - t2trans[0]) <= 1.2 && stopflag1 == 0)
		{
			if ((abs(pos[i][1] - t2trans[1]) <= .7) && pos[i][1] >= -.2)
			{
				if (hitflag == 0)
				{
					printf("\nHIT PLAYER 2!\n");
   ...
   for (int i = 0; i < NUM_PARTICLES; i++) 
	{
      ...
      pos2[i][2] += vel2[i][2] * TIMESTEP +
		...
		//printf("pos[i][1]: %f\n", pos[i][1]);
		//printf("t2trans[0]: %f\n", t2trans[0]);
		if (abs(pos2[i][0] - t1trans[0]) <= 1.2 && stopflag2 == 0)
		{
			if ((abs(pos2[i][1] - t1trans[1]) <= .7) && pos2[i][1] >= -.2)
			{
				if (hitflag2 == 0)
				{
					printf("\nHIT PLAYER 1!\n");
